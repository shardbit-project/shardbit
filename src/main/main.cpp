...
const string strMessageMagic = "Shardbit Signed Message:\n";
...
int64_t GetProofOfWorkReward(int nHeight, int64_t nFees)
{
    ...
    if (nHeight > 1000000) nSubsidy = 250 * COIN;

    if (nHeight > 1500000)
        nSubsidy >>= ((nHeight - 1000001) / 500000);
...
int64_t GetProofOfStakeReward(const CBlockIndex* pindexPrev, int64_t nCoinAge, int64_t nFees)
{
    ...
    if (pindexBest->nHeight > 1) nSubsidy = 500 * COIN; // instamine prevention
    if (pindexBest->nHeight > 1000) nSubsidy = 8000 * COIN;
    if (pindexBest->nHeight > 10000) nSubsidy = 4000 * COIN;
    if (pindexBest->nHeight > 100000) nSubsidy = 2000 * COIN;
    if (pindexBest->nHeight > 1000000) nSubsidy = 1000 * COIN;

    if (pindexBest->nHeight > 1500000)
        nSubsidy >>= ((pindexBest->nHeight - 1000001) / 500000);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        if (foundPaymentAndPayee) {
            ...
            if (IsProtocolV2(pindex->nHeight)) {
