...
int nAnonymizeShardbitAmount = 1000;
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Shardbit
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Shardbit
    // Mac: ~/Library/Application Support/Shardbit
    // Unix: ~/.shardbit
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Shardbit";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "shardbit.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "shardbitd.pid"));
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        else
        {
            ...
            if (!fDone)
            {
                ...
                {
                    fDone = true;
                    string strMessage = _("Warning: Please check that your computer's date and time are correct! If your clock is wrong SHARDBIT will not work properly.");
