...
bool CMasternodeConfig::read(boost::filesystem::path path) {
    ...
    for(std::string line; std::getline(streamConfig, line); )
    {
        ...
        if (!(iss >> alias >> ip >> privKey >> txHash >> outputIndex >> donation)) {
            ...
            if (!address.IsValid()) {
                LogPrintf("InvalidSHDBaddress in masternode.conf line: %s\n", line.c_str());
                streamConfig.close();
                return false;
            }
        }
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(CService(ip).GetPort() != 37451) {
                LogPrintf("Invalid port detected in masternode.conf: %s (must be 37451 for mainnet)\n", line.c_str());
                streamConfig.close();
                return false;
            }
        } else if(CService(ip).GetPort() == 37451) {
            LogPrintf("Invalid port detected in masternode.conf: %s (37451 must be only on mainnet)\n", line.c_str());
            streamConfig.close();
            return false;
        }
