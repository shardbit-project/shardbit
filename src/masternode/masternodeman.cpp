...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "dsee") { //DarkSend Election Entry
        ...
        if(Params().NetworkID() == CChainParams::MAIN){
            if(addr.GetPort() != 37451) return;
            if(!isIPV4) return;
        } else if(addr.GetPort() == 37451) return;
        ...
        if(fAcceptable){
            ...
            // should be at least not earlier than block when Shardbit tx got MASTERNODE_MIN_CONFIRMATIONS
