...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("shardbit"))
        return false;
...
{
    // Convert shardbit:// to shardbit:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("shardbit://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "shardbit:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Shardbit.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for Shardbit.lnk
...
{
    return GetAutostartDir() / "shardbit.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a shardbit.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=Shardbit\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the shardbit app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add shardbit app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Shardbit-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  shardbit-qt [" + tr("command-line options") + "]                     " + "\n";
