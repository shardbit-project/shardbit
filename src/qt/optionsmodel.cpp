...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeShardbitAmount"))
        settings.setValue("nAnonymizeShardbitAmount", 1000);
    nAnonymizeShardbitAmount = settings.value("nAnonymizeShardbitAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeShardbitAmount"))
        SoftSetArg("-anonymizeshardbitamount", settings.value("nAnonymizeShardbitAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeShardbitAmount:
            return QVariant(nAnonymizeShardbitAmount);
        default:
            return QVariant();
        }
    }
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeShardbitAmount:
            nAnonymizeShardbitAmount = value.toInt();
            settings.setValue("nAnonymizeShardbitAmount", nAnonymizeShardbitAmount);
            emit AnonymizeShardbitAmountChanged(nAnonymizeShardbitAmount);
            break;
        default:
            break;
        }
